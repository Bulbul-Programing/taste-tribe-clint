...
import { useGetAllFollowingQuery, useRemoveFollowerMutation } from '@/src/redux/Users/userManagementApi';
                ...
                data?.data?.length > 0 ? data?.data?.map((user: TUser) => (
                    <div key={user._id} className="flex flex-wrap border p-2 gap-x-3 items-center rounded transition-all hover:bg-slate-100">
                        ...
                        <div>
                            ...
                            <p>Phone: <span className=" font-medium">{user.phoneNumber.slice(0, 8)}***</span></p>
                        </div>
                        <div ><Button onClick={() => handleUnFollow(user._id)} size='sm' className='bg-[#1BEEA2]'>Un follow</Button></div>
                    </div>
                )) :
